...
|---|---|---|
| **Base URL** | `https://api.anthropic.com` | `https://api.tonbo.dev` |
...
- **Lower price for the session layer alone** — about 1/130th of Anthropic's bundled session-hour fee at the same workload (see [Overview → Why not Anthropic](/overview#why-not-anthropic-managed-agents)).
...
curl https://api.tonbo.dev/v1/sessions/$SESSION_ID/events \
